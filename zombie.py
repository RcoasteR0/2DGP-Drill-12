...
class Zombie:
    ...
    def runaway_to_boy(self, r=0.5):
        self.state = 'Walk'
        rx = self.x - play_mode.boy.x
        ry = self.y - play_mode.boy.y
        self.move_slightly_to(self.x + rx, self.y + ry)
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def build_behavior_tree(self):
        ...
        move_to_target_location = Sequence('how to target location', a1, a2)

        a3 = Action('Set random location', self.set_random_location)
        wander = Sequence('Wander', a3, a2)

        c1 = Condition('소년이 근처에 있는가?', self.is_boy_nearby, 7)
        c2 = Condition('소년의 공 개수 이상인가?', self.is_ball_more_than_boy)
